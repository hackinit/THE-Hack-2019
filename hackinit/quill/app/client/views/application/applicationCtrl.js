    ...
    function($scope, $rootScope, $state, $http, currentUser, Settings, Session, UserService, AutocompleteService){
      ...
      function _uploadResume() {
        ...
        if (files.length == 0) {
          ...
          $.ajax({
            type: 'PUT',
            url: '//api.thehack.org.cn/s3/upload/resume/hackinit/' + $scope.user._id + '_resume' + _getExtension(resume.name),
            data: formData,
            processData: false,
            contentType: false,
      ...
      function _waitForSuccess(token, success, failed) {
        $http.get('//api.thehack.org.cn/s3/status/' + token).then(function(response) {
